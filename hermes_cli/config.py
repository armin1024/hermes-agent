...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "AOPS_BOT_TOKEN", "AOPS_BOT_URL", "AOPS_BASE_URL", "AOPS_HOME_CHANNEL",
...
def migrate_aops_env_keys(env_path: Optional[Path] = None) -> bool:
    """Rename legacy AOPS_BASE_URL entries in .env to AOPS_BOT_URL.

    Existing users may upgrade with ``AOPS_BASE_URL`` already present in
    ``~/.hermes/.env``.  Runtime still reads the legacy key as a fallback, but
    setup/upgrade should also normalize the file so subsequent edits use the
    current ``AOPS_BOT_URL`` name.
    """
    if is_managed():
        return False
    ...
    lines = _sanitize_env_lines(original_lines)
    old_entries: list[tuple[int, str, str, str, str]] = []
    new_entries: list[tuple[int, str, str, str, str]] = []
    assignment_re = re.compile(
        r"^([ \t]*(?:export[ \t]+)?)(AOPS_BASE_URL|AOPS_BOT_URL)([ \t]*=[ \t]*)(.*?)(\r?\n?)$"
    )
    ...
    for index, line in enumerate(lines):
        ...
        if not match:
            continue
        prefix, key, separator, value, newline = match.groups()
        entry = (index, prefix, separator, value, newline or "\n")
        if key == "AOPS_BASE_URL":
    ...
    old_value = old_entries[-1][3]
    old_value_nonempty = bool(old_value.strip().strip("\"'"))
    remove_old_indices = {index for index, *_ in old_entries}
    migrated_lines = list(lines)

    if new_entries:
        new_has_value = any(value.strip().strip("\"'") for _, _, _, value, _ in new_entries)
        if old_value_nonempty and not new_has_value:
            index, prefix, separator, _, newline = new_entries[0]
            migrated_lines[index] = f"{prefix}AOPS_BOT_URL{separator}{old_value}{newline}"
    else:
        index, prefix, separator, _, newline = old_entries[0]
        migrated_lines[index] = f"{prefix}AOPS_BOT_URL{separator}{old_value}{newline}"
        remove_old_indices.discard(index)

    migrated_lines = [
        line for index, line in enumerate(migrated_lines)
        if index not in remove_old_indices
    ]

    if migrated_lines == original_lines:
        return False
    ...
    if old_value_nonempty and not os.environ.get("AOPS_BOT_URL"):
        os.environ["AOPS_BOT_URL"] = old_value.strip().strip("\"'")
    invalidate_env_cache()
    return True
