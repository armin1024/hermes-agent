...
def _cron_items() -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
    ...
    payload = cron_jobs.load_jobs_payload()
        ...
        items.append(
            {
                ...
                "updatedAtMs": _to_ms(job.get("updated_at") or payload.get("updated_at")),
...
def _read_cron_history(command_text: str, args: list[str]) -> str:
    ...
    try:
        entries = cron_jobs.get_job_history(job_id, limit=None)
...
def run_curator_command(raw_args: str) -> tuple[int, str]:
    ...
    return int(exit_code or 0), text or "Command returned no output."
