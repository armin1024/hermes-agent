...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if aops_bot_token or aops_base_url:
        ...
        if aops_allow_from:
            extra["allow_from"] = [item.strip() for item in aops_allow_from.split(",") if item.strip()]
        aops_trusted = os.getenv("AOPS_TRUSTED_AGENT_KEY_FROM", "").strip()
        if aops_trusted:
            extra["trusted_agent_key_from"] = [item.strip() for item in aops_trusted.split(",") if item.strip()]
